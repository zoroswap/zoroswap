...
async fn e2e_private_deposit_withdraw_test() -> Result<()> {
    // Use a fresh store to avoid stale SMT forest data from accumulated test state.
    // The NoteScreener checks all accounts in the store against output notes, and stale
    // storage map roots in the in-memory SMT forest cause RootNotInStore errors.
