...
async fn e2e_position_swap() -> Result<()> {
    ...
    let res: AddPositionResponse = serde_json::from_str(&res[0])?;
    ...
    let reclaimed_note = get_position_note(
...
#[tokio::test]
async fn e2e_3_assets_position_swap() -> Result<()> {
    let filter_layer = EnvFilter::try_from_default_env().unwrap_or_else(|_| {
        EnvFilter::new(
            "info,miden_client=warn,rusqlite_migration=warn,h2=warn,rustls=warn,hyper=warn",
        )
    ...
    let pool0 = config.liquidity_pools[0];
    let pool1 = config.liquidity_pools[1];
    let pool2 = config.liquidity_pools[2];
    ...
    let initial_pool0 = *zoro_pool.pool_states().get(&pool0.faucet_id).unwrap();
    let initial_pool1 = *zoro_pool.pool_states().get(&pool1.faucet_id).unwrap();
    let initial_pool2 = *zoro_pool.pool_states().get(&pool2.faucet_id).unwrap();
    // ---------------------------------------------------------------------------------
    println!("\n\t[STEP 1] Fund user wallet\n");
    let amount = 500000;
    miden_client
        ...
        .await?;
    miden_client
        .mint_asset(pool1.faucet_id, *account.id(), amount)
        .await?;
    miden_client
        .mint_asset(pool2.faucet_id, *account.id(), amount)
        .await?;
    ...
    let user_balance0 = account.get_balance(&pool0.faucet_id).await?;
    let user_balance1 = account.get_balance(&pool1.faucet_id).await?;
    let user_balance2 = account.get_balance(&pool2.faucet_id).await?;
    info!("Minted: {amount} to the test account. New balance: {user_balance0}");
    ...
    let note = TrustedNote::new(
        NoteInstructions {
            note_kind: NoteKind::Position,
            attached_assets: vec![
                FungibleAsset::new(pool0.faucet_id, amount)?,
                FungibleAsset::new(pool1.faucet_id, amount)?,
                FungibleAsset::new(pool2.faucet_id, amount)?,
            ],
            asset_input: None,
            beneficiary: *account.id(),
            amount_input: amount,
